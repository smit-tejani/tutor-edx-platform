...
class LearnerPathwayProgress(TimeStampedModel):
    ...
    def get_learner_course_status(user, course):
        ...
        """
        ##TODO: Need to find the courserun key from the course key
        learner_course_grade = PersistentCourseGrade.objects.filter(user_id=user.id, course_id=course['key'])
        if learner_course_grade.passed_timestamp:
    ...
    def update_pathway_progress(self):
        ...
        """
        pathway_steps = self.learner_pathway_progress.steps or []
